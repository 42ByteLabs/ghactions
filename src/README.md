...
By default, we use `rustls-aws-lc` as the TLS cryptography library.

If you want to use another you must do the following:

1. Disable the default features using `default-features = false`
2. Add the `octocrab` feature and then one of the following:
    - `rustls-aws-lc` (default) - Rustls with AWS-LC crypto provider
    - `rustls-ring` - Rustls with Ring crypto provider  
    - `native-tls` - Native TLS (OpenSSL on Linux, Secure Transport on macOS, Schannel on Windows)
